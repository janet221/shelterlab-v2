-- Once students have joined, the join code is their stable classroom identity.
-- School metadata remains editable and is read dynamically by every student.
begin;

create or replace function public.shelterlab_save_class(
  p_class_id uuid,
  p_school_id text,
  p_school_name text,
  p_county text,
...
begin
  ...
  if p_class_id is null then
    ...
    if (select count(*) from public.profiles where class_id = p_class_id and role = 'student') > p_student_count then
      raise exception 'Class capacity below enrollment' using errcode = '23514';
    end if;
    if exists(select 1 from public.profiles where class_id = p_class_id and role = 'student') and exists(
      select 1 from public.classes where id = p_class_id and class_code <> v_class_code) then
      raise exception 'Cannot change enrolled class code' using errcode = '23514';
    end if;
    update public.classes
      set name=left(p_school_name || ' ' || p_grade,100),
          school_id=p_school_id, school_name=p_school_name, county=p_county, grade=p_grade,
