...
const fieldClass = "mt-2 w-full rounded-2xl border border-[#dbc69f] bg-[#fffdf8] px-4 py-3.5 text-[#332a22] shadow-sm outline-none transition focus:border-[#b8893e] focus:ring-4 focus:ring-[#d9b45b]/20 disabled:bg-[#f3eadc]";
...
export default function ResetPasswordForm() {
  ...
  if (complete) return <section className="rounded-[2rem] border border-[#e3cfaa] bg-[#fffdf8] p-7 shadow-sm"><p role="status" className="text-sm font-medium text-[#5d4729]">密碼已更新，請使用新密碼登入。</p><Link href="/auth" className="mt-5 inline-flex rounded-2xl bg-[#c99d45] px-5 py-3 font-bold text-[#30251b] hover:bg-[#ddb85f]">返回登入</Link></section>;
  ...
  return <form className="space-y-5 rounded-[2rem] border border-[#e3cfaa] bg-[#fffdf8] p-5 shadow-[0_24px_70px_-40px_rgba(111,78,34,0.45)] sm:p-7" onSubmit={updatePassword}><label className="block text-sm font-bold text-stone-700">新密碼<input className={fieldClass} name="password" type="password" minLength={12} maxLength={128} autoComplete="new-password" required /><span className="mt-2 block text-xs font-normal text-stone-500">建議使用 12 字元以上，或一段您容易記憶的長句子</span></label><label className="block text-sm font-bold text-stone-700">再次輸入新密碼<input className={fieldClass} name="confirmation" type="password" minLength={12} maxLength={128} autoComplete="new-password" required /></label>{error && <p role="alert" className="rounded-xl bg-red-50 px-4 py-3 text-sm font-medium text-red-800">{error}</p>}<button disabled={busy} className="w-full rounded-2xl bg-[#c99d45] px-5 py-3.5 font-bold text-[#30251b] transition hover:bg-[#ddb85f] disabled:cursor-not-allowed disabled:opacity-60">{busy ? "更新中…" : "更新密碼"}</button></form>;
}
