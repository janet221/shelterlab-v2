...
export default async function AuthPage({ searchParams }: { searchParams: Promise<{ role?: string }> }) {
  const { role } = await searchParams;
  const roleLabel = role === "teacher" ? "教師" : role === "student" ? "學生" : "學習帳號";
  return (
    <PublicPageShell>
      <main className="min-h-[70vh] bg-[radial-gradient(circle_at_top,rgba(227,184,90,0.2),transparent_42%),linear-gradient(180deg,#fffaf0,#f6eddc)] px-5 py-12 sm:py-16">
        <div className="mx-auto max-w-lg">
          <p className="font-bold tracking-wide text-[#9b7130]">ShelterLab · {roleLabel}</p>
