...
export default function WeekFiveExperience() {
  ...
  const next = async () => { if (!canContinue) return; setFeedback(null); if (saved.stage < 5) { const stage = saved.stage + 1; setSaved((current) => ({ ...current, stage, furthest: Math.max(current.furthest, stage) })); scrollTo({ top: 0, behavior: "smooth" }); } else if (await completeWeek(5)) { setSaved((current) => ({ ...current, completed: true })); router.push("/student"); } else { setFeedback({ kind: "wrong", title: "尚未送出", body: "請確認網路後再送出；目前作答都已保留。" }); } };
