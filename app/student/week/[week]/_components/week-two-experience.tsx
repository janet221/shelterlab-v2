...
export default function WeekTwoExperience() {
  ...
  const next = async () => { if (!canContinue) return; setFeedback(null); if (saved.stage < 5) { const stage = saved.stage + 1; setSaved((current) => ({ ...current, stage, furthest: Math.max(current.furthest, stage) })); scrollTo({ top: 0, behavior: "smooth" }); } else if (await completeWeek(2)) { setSaved((current) => ({ ...current, completed: true })); router.push("/student"); } else { setFeedback({ kind: "wrong", title: "尚未送出", body: "請確認網路後再送出；目前作答都已保留。" }); } };

  if (!ready) return <main className={base.experience}><div className={base.loading}>正在整理第二週的責任檔案…</div></main>;
  if (saved.completed) return <main className={base.experience}><div className={base.shell}><nav className={base.topbar}><Link href="/student" className={base.backLink}>← 返回六週地圖</Link><span className={base.weekStamp}>WEEK 02 · 稽核中</span></nav><article className={`${base.stagePaper} ${styles.completionPaper}`}><section className={styles.completionHero}><p>第二週作答完成</p><h1>已送交教師稽核</h1><div className={styles.rewardStage}><span aria-hidden="true" /><img src={REWARD.image} alt="審核通過後可獲得的寶物" /></div><h2>通過後獲得：{REWARD.name}</h2><p>作答已鎖定；教師通過後，寶物與下一週會一起解鎖。</p><div className={styles.completionActions}><button type="button" className={base.paperButton} onClick={() => router.push("/student")}>返回地圖等待審核</button></div></section></article></div></main>;
