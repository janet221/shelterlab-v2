...
export default function WeekSixExperience(){
 ...
 const complete=async()=>{const r=draft.actionRecord;if(!r.actionType.trim()||!r.plannedDate||!r.adultSupportNote.trim()||!r.conditionsToConfirm.trim()||!r.nextStep.trim()||!r.alternativePlan.trim()||r.reflection.trim().length<20){setNotice("請補齊行動、日期、成人協助、確認條件、下一步、替代方案與至少 20 字的行前預期成果。");return}setNotice("正在送交教師稽核…");if(await completeWeek(6)){setDraft(d=>({...d,status:"completed"}));window.location.assign("/student")}else setNotice("尚未成功送出；請確認網路後再試，目前作答都已保留。");};
