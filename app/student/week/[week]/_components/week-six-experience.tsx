...
import { organizationRepository } from "@/lib/action-opportunities/browser-repository";
import { WEEK_SIX_DRAFT_KEY, WEEK_SIX_LEGACY_DRAFT_KEY, type ActionFilter, type ActionProfile, type ContactMethod, type Coordinates, type ResourceCategoryFilter, type WeekSixJourneyDraft } from "@/lib/action-opportunities/types";
import { areSafetyAnswersCorrect, buildContactDrafts, createWeekSixDraft, dedupeActionOrganizations, filterActionMatches, matchActionOrganizations, parseWeekSixDraft, requiredComparisonCount } from "@/lib/action-opportunities/repository";
...
export default function WeekSixExperience(){
 ...
 useEffect(()=>{const controller=new AbortController();const read=<T,>(url:string)=>fetch(url,{signal:controller.signal}).then(r=>{if(!r.ok)throw Error();return r.json() as Promise<T>});Promise.allSettled([read<AdoptionSnapshot>("/api/open-data/adoptions"),read<ShelterStatsSnapshot>("/api/open-data/shelter-stats"),read<ShelterNeedsSnapshot>("/api/open-data/shelter-needs"),read<SchoolDirectorySnapshot>("/api/open-data/schools")]).then(results=>{if(controller.signal.aborted)return;const a=results[0].status==="fulfilled"?results[0].value:fallbackAdoptions(),s=results[1].status==="fulfilled"?results[1].value:fallbackStats(),n=results[2].status==="fulfilled"?results[2].value:fallbackNeeds(),sc=results[3].status==="fulfilled"?results[3].value:fallbackSchools();organizationRepository.upsertMany(a.organizations);setAdoptions(a);setStats(s);setNeeds(n);setSchools(sc);setDraft(d=>({...d,openDataDate:a.source.updatedAt}));if(results.some(result=>result.status==="rejected"))setDataError("部分即時資料暫時無法使用，已切換到有日期且標示來源的備援快照；地圖與六個環節仍可完成。")});try{const restored=parseWeekSixDraft(learningStorage.getItem(WEEK_SIX_DRAFT_KEY))??parseWeekSixDraft(learningStorage.getItem(WEEK_SIX_LEGACY_DRAFT_KEY));if(restored)setDraft(restored)}catch{}setHydrated(true);return()=>controller.abort()},[]);
 useEffect(()=>{if(!hydrated)return;try{learningStorage.setItem(WEEK_SIX_DRAFT_KEY,JSON.stringify({...draft,updatedAt:new Date().toISOString()}));if(learningStorage.getItem(WEEK_SIX_LEGACY_DRAFT_KEY))learningStorage.removeItem(WEEK_SIX_LEGACY_DRAFT_KEY)}catch{}},[draft,hydrated]);
