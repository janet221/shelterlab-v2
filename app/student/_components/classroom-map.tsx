...
export default function ClassroomMap() {
  ...
  return <><p role="alert" className="relative z-[90] bg-white px-5 text-red-800">{error}</p>{!progress && !error && <p className="p-10">正在讀取地圖進度…</p>}{progress?.enrolled ? <StudentMapDynamic classCode={progress.classCode} progress={{ weeks: ([1, 2, 3, 4, 5, 6] as const).map(week => progress.weeks.find(w => w.week === week) || { week, status: "locked" }) }} /> : progress && <main className="mx-auto max-w-lg px-5 py-16"><h1 className="text-3xl font-bold">尚未綁定班級</h1><p className="my-5">學生帳號會在註冊時綁定班級。請登出後以教師提供的班級代碼重新註冊，或聯絡教師協助。</p></main>}</>;
}
