...
export function ClassroomNav({ teacher = false }: { teacher?: boolean }) {
  return <nav className="flex flex-wrap items-center gap-4 border-b border-stone-200 bg-white px-5 py-4 text-sm font-bold" aria-label="班級導覽"><Link href="/">ShelterLab</Link>{teacher ? <><Link href="/teacher">在地課程工作台</Link><Link href="/teacher/reviews">待審作業</Link><Link href="/teacher/settings">設定</Link></> : <Link href="/student">學習地圖</Link>}<button className="ml-auto underline" onClick={async () => { try { await api("/api/auth/logout", {}); window.location.href = "/auth"; } catch { window.alert("登出失敗，請重試。"); } }}>登出</button></nav>;
}
