...
export function ClassroomNav({ teacher = false }: { teacher?: boolean }) {
  return <nav className="flex flex-wrap items-center gap-4 border-b border-[#ded7cc] bg-[#fffdf8] px-5 py-4 text-sm font-bold" aria-label="班級導覽"><Link href="/">ShelterLab</Link>{teacher ? <><Link href="/teacher/reviews">學習進度追蹤</Link><Link href="/teacher/settings">設定</Link></> : <Link href="/student">學習地圖</Link>}<button className="ml-auto underline" onClick={async () => { try { const supabase = createBrowserSupabaseClient(); const { error } = await supabase.auth.signOut(); if (error) throw error; await api("/api/auth/logout", {}).catch(() => undefined); window.location.href = "/auth"; } catch { window.alert("登出失敗，請重試。"); } }}>登出</button></nav>;
}
