...
export default function DataLens({ metadata: m, annotation }: { metadata: DataLensMetadata; annotation?: AnnotationTarget }) {
  ...
  return <details className="mt-4 rounded-xl border border-teal-200 bg-teal-50/60 p-4 text-sm" onToggle={(event) => setOpen(event.currentTarget.open)}>
    <summary className="cursor-pointer font-bold text-teal-900">🔍 資料透鏡</summary>
    <dl className="mt-4 grid gap-3 sm:grid-cols-2">
      ...
      <div><dt className="font-bold text-teal-900">可以說明</dt><dd>{m.canExplain}</dd></div>
    ...
    {annotation && <section className="mt-5 border-t border-teal-200 pt-5" aria-label="教育標註">
      <label className="block font-bold text-teal-950">教育標註（儲存後可供未來學生教材同步）<textarea className={fieldClass} rows={4} maxLength={2000} value={note} disabled={!loaded || busy} placeholder={loaded ? "例如：提醒學生這是資料建立日趨勢，不是真實入所趨勢。" : "正在讀取既有標註…"} onChange={(event) => setNote(event.target.value)} /></label>
      <div className="mt-3 flex flex-wrap items-center gap-3">
        <button type="button" className="rounded-xl bg-teal-800 px-4 py-2 font-bold text-white disabled:opacity-50" disabled={!loaded || busy} onClick={async () => {
          setBusy(true);
          setError("");
          try {
        ...
        {updatedAt && <p role="status" className="text-xs text-teal-900">已儲存：{new Intl.DateTimeFormat("zh-TW", { dateStyle: "medium", timeStyle: "short" }).format(new Date(updatedAt))}</p>}
