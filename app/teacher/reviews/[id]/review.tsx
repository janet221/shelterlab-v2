...
export default function Review({ id }: { id: string }) {
  ...
  async function decide() { if (!record) return; setBusy(true); setError(""); try { await api(`/api/classroom/reviews/${id}`, { version: record.version, generation: record.generation, decision: "approve", feedback }); window.location.href = "/teacher/reviews"; } catch (e) { setError((e as Error).message); } finally { setBusy(false); } }
  return <main className="mx-auto max-w-5xl space-y-6 px-5 py-10"><Link className="underline" href="/teacher/reviews">← 返回待審清單</Link><h1 className="text-3xl font-bold">逐題檢視與批改</h1>{error && <div role="alert" className="fixed right-5 top-20 z-[100] max-w-sm rounded-2xl border border-red-300 bg-red-50 px-5 py-4 text-red-900 shadow-xl">審核失敗：{error}</div>}{record && set ? <><p>{record.enrollment.student.displayName} · 帳號 {record.enrollment.student.id.slice(0, 8)} · {courseWeekLabel(record.week)} · {record.status === "pending" ? "等待審核中" : "目前不可批改"}</p><CaseComparison set={set} />{set.questions.map((q, i) => { const answer = answers?.find(a => a.questionId === q.id); return <article key={q.id} className="rounded-2xl border bg-white p-6"><h2 className="text-xl font-bold">{i + 1}. {q.prompt}</h2><p className="my-3 text-sm">後端計算依據：{q.fact}</p><p className="font-bold">學生選用紀錄：{answer?.selectedAnimalIds.join("、") || "未選擇"}</p><p className="mt-4 whitespace-pre-wrap rounded-xl bg-stone-50 p-4">{answer?.text || "尚未作答"}</p></article>; })}<DataLens metadata={set.metadata} /><label className="block font-bold">教師回饋<textarea value={feedback} onChange={e => setFeedback(e.target.value)} maxLength={3000} rows={4} className={fieldClass} /></label><div className="flex gap-4"><button className={buttonClass} disabled={busy || record.status !== "pending"} onClick={decide}>{busy ? "審核中…" : record.week === 6 ? "審核通過並完成課程" : "審核通過並解鎖下一週"}</button></div></> : !error && <p>載入作業中…</p>}</main>;
}
