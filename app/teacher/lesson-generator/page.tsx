...
function Bars({ points, suffix = "筆" }: { points: LessonChartPoint[]; suffix?: string }) {
  ...
  return <div className="space-y-3">{points.map((point) => <div key={point.label} className="grid grid-cols-[7rem_1fr_5rem] items-center gap-3 text-sm">
    ...
    <div className="h-5 overflow-hidden rounded-full bg-stone-100"><div className="h-full rounded-full bg-[#9b8d80]" style={{ width: `${point.count / max * 100}%` }} /></div>
...
export default function LessonGeneratorPage() {
  ...
  return <main className="mx-auto max-w-7xl space-y-8 px-5 py-10">
    <header className="rounded-3xl bg-gradient-to-br from-[#51463d] to-[#8a796a] p-8 text-white">
      <p className="font-bold text-[#efe4d2]">教師專屬 · 在地動保工作台</p>
      <h1 className="mt-2 text-3xl font-bold sm:text-4xl">在地教案產生器</h1>
      <p className="mt-4 max-w-3xl leading-7 text-[#fbf7ef]">所有數字均由伺服器解析專案內的農業部開放資料快照後計算。個案依固定規則選取，不使用隨機亂數，也不以 AI 生成統計數字。</p>
    </header>
    ...
    {loading && <p role="status" className="rounded-2xl bg-[#f3eee7] p-5 text-[#51463d]">正在解析開放資料並執行確定性計算…</p>}
    ...
    {lesson?.available && classroom && <>
      <section className="rounded-2xl border bg-white p-6">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-bold text-[#7f7165]">{lesson.county} · 快照概覽</p><h2 className="mt-1 text-2xl font-bold">由真實資料計算的教案摘要</h2></div><span className="rounded-full bg-[#e8dfd5] px-4 py-2 text-sm font-bold text-[#685b50]">零幻覺公式計算</span></div>
      ...
      <section className="rounded-2xl border bg-white p-6"><h2 className="text-xl font-bold">確定性個案對照 · {lesson.selectedCases.length} 例</h2><p className="mt-2 text-stone-600">{lesson.selectionBasis}</p><div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{lesson.selectedCases.map((item) => <article key={item.id} className="rounded-2xl border border-stone-200 p-5"><p className="text-xs font-bold text-[#7f7165]">真實識別碼 {item.id}</p><h3 className="mt-2 font-bold">{item.shelter}</h3><dl className="mt-3 space-y-2 text-sm"><div><dt className="text-stone-500">毛色／體型</dt><dd>{item.colour}／{item.body}</dd></div><div><dt className="text-stone-500">品種／年齡</dt><dd>{item.variety}／{item.age}</dd></div><div><dt className="text-stone-500">資料建立日</dt><dd>{item.createdDate || "缺漏"}</dd></div><div><dt className="text-stone-500">推定留所天數</dt><dd className="text-2xl font-bold">{item.days === null ? "無法計算" : `${item.days} 天`}</dd></div></dl></article>)}</div><DataLens metadata={lesson.lenses.cases} annotation={lensTarget("cases")} /></section>

      <section className="rounded-2xl bg-[#51463d] p-6 text-white"><h2 className="text-2xl font-bold">探究問題卡</h2><ol className="mt-5 space-y-5">{lesson.inquiryQuestions.map((item, index) => <li className="rounded-2xl bg-white/10 p-5" key={item.prompt}><p className="text-sm text-[#efe4d2]">資料事實 {index + 1}：{item.fact}</p><p className="mt-2 text-lg font-bold">{item.prompt}</p></li>)}</ol></section>
    </>}
  </main>;
}
