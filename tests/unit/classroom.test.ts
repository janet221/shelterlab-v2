...
describe("authoritative COA classroom evidence", () => {
  ...
  it("accepts the teacher class code settings and rejects address or privilege fields", () => {
    const data = { teacherName: "王老師", classCode: "SHELTER-2026", schoolId: "120303", county: "高雄市", grade: "高一", studentCount: 30, plannedWeeks: 6 };
  ...
  it("shows only gradable answers and persists per-question teacher comments", () => {
    ...
    expect(isGradableAuditEntry(base)).toBe(true);
  ...
  it("uses the approved six-week labels and rejects shortened courses", () => {
    ...
    expect(settingsSchema.safeParse({ teacherName: "王老師", classCode: "SHELTER-2026", schoolId: "120303", county: "高雄市", grade: "高一", studentCount: 30, plannedWeeks: 5 }).success).toBe(false);
  });
});
